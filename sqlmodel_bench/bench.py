...
from sqlmodel_bench.models import User, Post, Tag, PostTag


class SQLModelBenchmark(Benchmark):
    """Benchmark implementation for SQLModel (async)."""
    ...
    async def setup(self, db_url: str) -> None:
        """Initialize SQLModel async engine."""
        # Convert sync URL to async URL (same as SQLAlchemy)
    ...
    async def filter_in(self, ids: list[int]) -> list:
        ...
        async with self.SessionLocal() as session:
            result = await session.execute(select(User).where(col(User.id).in_(ids)))
            return result.scalars().all()
